// swift-tools-version:5.6

import PackageDescription

/// Canonical manifest copied into the distribution staging package by build.sh.
let package = Package(
    name: "AppBoxSDK",
    platforms: [.iOS(.v13)],
    products: [
        .library(name: "AppBoxSDK", targets: ["AppBoxSDKSupport"]),
        .library(name: "AppBoxHealthSDK", targets: ["AppBoxHealthSDK"]),
        .library(name: "AppBoxPushSDK", targets: ["AppBoxPushSDK"]),
        .library(name: "AppBoxInappMessageSDK", targets: ["AppBoxInappMessageSDK"]),
        .library(name: "AppBoxSnsLoginSDK", targets: ["AppBoxSnsLoginSDK"]),
    ],
    dependencies: [
        .package(url: "https://github.com/firebase/firebase-ios-sdk.git", from: "11.12.0"),
        .package(url: "https://github.com/google/GoogleSignIn-iOS", from: "9.0.0"),
        .package(url: "https://github.com/kakao/kakao-ios-sdk", from: "2.26.0"),
        .package(url: "https://github.com/naver/naveridlogin-sdk-ios-swift", from: "5.1.0"),
        .package(url: "https://github.com/AppsFlyerSDK/AppsFlyerFramework", .upToNextMinor(from: "6.17.6")),
        .package(url: "https://github.com/airbnb/lottie-spm.git", from: "4.6.0"),
    ],
    targets: [
        .binaryTarget(name: "AppBoxSDK", path: "./Sources/AppBoxSDK/AppBoxSDK.xcframework"),
        .binaryTarget(name: "AppBoxCoreSDK", path: "./Sources/AppBoxCoreSDK/AppBoxCoreSDK.xcframework"),
        .target(
            name: "AppBoxSDKSupport",
            dependencies: [
                "AppBoxSDK",
                "AppBoxCoreSDK",
                "AppBoxWebViewSDK",
                "AppBoxInappMessageSDK",
                "AppBoxWatermarkSupport",
                .product(name: "Lottie", package: "lottie-spm"),
                .product(name: "AppsFlyerLib", package: "AppsFlyerFramework"),
            ],
            path: "Sources/AppBoxSDKSupport"
        ),
        .target(
            name: "AppBoxHealthSDK",
            dependencies: ["AppBoxCoreSDK"],
            path: "Sources/AppBoxHealthSDK",
            resources: [.process("Resources/PrivacyInfo.xcprivacy")]
        ),
        .target(
            name: "AppBoxPushSDK",
            dependencies: [
                "AppBoxCoreSDK",
                "AppBoxWatermarkSupport",
                .product(name: "FirebaseMessaging", package: "firebase-ios-sdk"),
            ],
            path: "Sources/AppBoxPushSDK",
            resources: [.process("Resources/PrivacyInfo.xcprivacy")]
        ),
        .target(
            name: "AppBoxInappMessageSDK",
            dependencies: ["AppBoxCoreSDK", "AppBoxWatermarkSupport"],
            path: "Sources/AppBoxInappMessageSDK",
            resources: [.process("Resources/PrivacyInfo.xcprivacy")]
        ),
        .binaryTarget(
            name: "AppBoxWatermarkSupport",
            path: "./Sources/AppBoxWatermarkSupport/AppBoxWatermarkSupport.xcframework"
        ),
        .binaryTarget(
            name: "AppBoxWebViewSDK",
            path: "./Sources/AppBoxWebViewSDK/AppBoxWebViewSDK.xcframework"
        ),
        .target(
            name: "AppBoxSnsLoginSDK",
            dependencies: [
                "AppBoxCoreSDK",
                .product(name: "FirebaseCore", package: "firebase-ios-sdk"),
                .product(name: "FirebaseAuth", package: "firebase-ios-sdk"),
                .product(name: "GoogleSignIn", package: "GoogleSignIn-iOS"),
                .product(name: "KakaoSDKUser", package: "kakao-ios-sdk"),
                .product(name: "KakaoSDKAuth", package: "kakao-ios-sdk"),
                .product(name: "KakaoSDKCommon", package: "kakao-ios-sdk"),
                .product(name: "NidThirdPartyLogin", package: "naveridlogin-sdk-ios-swift"),
            ],
            path: "Sources/AppBoxSnsLoginSDK"
        ),
    ]
)
