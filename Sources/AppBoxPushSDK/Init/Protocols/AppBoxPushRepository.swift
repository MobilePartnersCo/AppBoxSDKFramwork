...
class AppBoxPushRepository: NSObject, AppBoxPushProtocol {
    ...
    func appBoxPushSendToken(pushYn: String, completion: @escaping (Bool, Bool) -> Void) {
        guard CorePushAgreement.isValid(pushYn) else {
            completion(true, false)
            return
        }
        let requestGeneration = agreementReconciler.beginExplicitRequest()
        guard let _ = FirebaseApp.app() else {
            debugLog("push init fail")
        ...
        if pushYn == "Y" {
            self.appBoxPushRequestPermissionForNotifications { permissionResult in
                if !permissionResult {
                    if !self.agreementReconciler.isLatestExplicitRequest(requestGeneration) {
                ...
                Messaging.messaging().token { token, error in
                    ...
                    self.agreementReconciler.requestExplicit(
                        value: pushYn,
                        token: pushToken,
